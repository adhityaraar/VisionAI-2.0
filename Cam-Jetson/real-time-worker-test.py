...
model = YOLO(f"{HOME}/models/yolo8-v1.pt", 'cuda')
...
def main():
    try:
        ...
        while True:
            ...
            print(f"Frame time: {frame_time:.2f} s, Pre-inference time: {pre_inference_time - start_time:.2f} s, Inference time: {inference_time:.2f} s, Post-inference time: {end_time - post_inference_time:.2f} s, FPS: {fps:.2f}")
